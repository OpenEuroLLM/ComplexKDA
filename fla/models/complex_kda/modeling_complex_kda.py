...
from fla.layers.complex_kda_layer import ComplexKimiDeltaAttention, _init_beta_projection_spread_, init_dt_bias, is_signed
...
from fla.modules import GatedMLP as ComplexKDAMLP
...
class ComplexKDABlock(GradientCheckpointingLayer):
    def __init__(self, config: ComplexKDAConfig, layer_idx: int):
        ...
        if attn_spec is not None:
            # `qk_norm`, `output_gate` and `use_rope` are read with .get: they are
            # not fields of HybridAttentionSpec, and normalize_hybrid_attention_config
            # preserves unknown keys by design. Their defaults are Attention's own,
            # so a spec that omits them builds exactly what it built before.
            #
            # The 1.3B hybrids need the last two. Their attention layers are GATED
            # and NoPE (Qwen3-Next's sigmoid on the attention output, no rotary --
            # the linear layers carry position), which is what the training backend
            # builds. Without these lines an exported hybrid is a DIFFERENT model:
            # rotary where the run had none, and no g_proj at all. The missing
            # g_proj makes a strict load fail loudly; the rotary would not, which
            # is the reason to pass both here rather than to check them downstream.
            self.attn = Attention(
                hidden_size=config.hidden_size,
                num_heads=attn_spec["num_heads"],
                num_kv_heads=attn_spec["num_kv_heads"],
                qkv_bias=attn_spec["qkv_bias"],
                qk_norm=attn_spec.get("qk_norm", False),
                output_gate=attn_spec.get("output_gate", False),
                use_rope=attn_spec.get("use_rope", True),
...
class ComplexKDAPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(
        ...
        if isinstance(module, ComplexKimiDeltaAttention) and next(module.parameters()).device.type != "meta":
            with torch.no_grad():
                if not getattr(module.A_log, '_is_hf_initialized', False):
                    module.A_log.zero_()
                if not getattr(module.dt_bias, '_is_hf_initialized', False):
                    dt = torch.exp(
                        nn.init.uniform_(module.dt_bias.new_empty(module.dt_bias.shape))
                    ...
                    # re-apply spread's f_proj rescale: the nn.Linear branch
                    # below has already re-initialised f_proj by this point.
                    if is_signed(module.gate) and module.gate_init_style == "spread":
                        module.f_proj[-1].weight.mul_(0.1)
                module.dt_bias._is_hf_initialized = True
                if module.beta_init_style == "spread":
                    _init_beta_projection_spread_(module.b_proj)
...
class ComplexKDAModel(ComplexKDAPreTrainedModel):
    def __init__(self, config: ComplexKDAConfig):
        ...
        self.layers = nn.ModuleList([ComplexKDABlock(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
    ...
    def forward(
        ...
        **kwargs: Unpack[dict],
        ...
        if output_attentions:
            warnings.warn("`ComplexKDAModel` does not `output_attentions` now, setting it to `False`.")
...
class ComplexKDAForCausalLM(ComplexKDAPreTrainedModel, FLAUnsupportedCacheGenerationMixin):
