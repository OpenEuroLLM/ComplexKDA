...
def _cache_seen_tokens(cache) -> int:
    """Tokens a cache has already consumed.

    `len(cache)` counts LAYERS, not tokens, so it is already nonzero for a
    freshly built cache. Using it to decide "has the prompt been fed yet?"
    slices the prompt down to its last token on the very first forward, and the
    recurrent state then never sees the prompt at all.
    """
    if cache is None:
        return 0
    ...
    try:
        return int(getter())
    except TypeError:          # some versions require a layer index
        return int(getter(0))
        ...
        **kwargs,
        ...
        if _IS_TRANSFORMERS_4_56_PLUS:
            ...
            if past_key_values is not None:
                ...
                elif _cache_seen_tokens(past_key_values) > 0:
            ...
            if _cache_seen_tokens(past_key_values) > 0:
                input_ids = input_ids[:, -1:]
            # if `inputs_embeds` are passed, we only want to use them in the 1st generation step
            if inputs_embeds is not None and _cache_seen_tokens(past_key_values) == 0:
                model_inputs = {'inputs_embeds': inputs_embeds}
            else:
