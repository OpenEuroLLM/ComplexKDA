...
from fla.ops.kda.gauge import (
    gauge_last,
    gauge_state,
    kda_sign_cumprod,
    l2norm_gauge_bwd,
    l2norm_gauge_fwd,
    l2norm_gauge_pair_bwd,
    l2norm_gauge_pair_fwd,
)
from fla.ops.kda.implementation import resolve_implementation, strict_selection
...
class ChunkKDAFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        p = None
        if sign is not None:
            if (implementation.parallel_parity and cu_seqlens is None and 8192 <= q.shape[1] <= 16384
                    and q.shape[2] * q.shape[3] == 2048):
                from fla.ops.kda.backends.optimized.parity import sign_cumprod_parallel
                p = sign_cumprod_parallel(sign, block_t=128, block_d=64)
            else:
                p = kda_sign_cumprod(sign, cu_seqlens)
        q_rstd, k_rstd = None, None
        if use_qk_l2norm_in_kernel:
            if implementation.paired_norm and (p is not None or implementation.fuse_norm):
                q, q_rstd, k, k_rstd = l2norm_gauge_pair_fwd(q, k, p)
            elif p is not None:
                q, q_rstd = l2norm_gauge_fwd(q, p)
                k, k_rstd = l2norm_gauge_fwd(k, p)
            else:
                q, q_rstd = l2norm_fwd(q)
                k, k_rstd = l2norm_fwd(k)
        elif p is not None:
            q, k = q * p.to(q.dtype), k * p.to(k.dtype)
        ...
        # the recurrence runs in the gauged frame; hand the state back in the true one
        p_last = None
        if p is not None and final_state is not None:
            p_last = gauge_last(p, cu_seqlens)
            final_state = gauge_state(final_state, p_last, state_v_first)
        ...
        ctx.save_for_backward(
            ...
            w, u, qg, kg, v_new, h,
            initial_state, cu_seqlens, chunk_indices, p, p_last
        )
    ...
    def backward(
        ...
        # `gauge_state` is its own inverse, so this re-enters the gauged frame
        if p_last is not None and dht is not None:
            dht = gauge_state(dht, p_last, ctx.state_v_first)

        fuse_norm_gauge_bwd = ctx.implementation.fuse_norm
        dq, dk, dv, db, dg, dh0, dA, dbias = chunk_kda_bwd(
            ...
            v_new=v_new,
            h=h,
            parity=p if fuse_norm_gauge_bwd else None,
            q_rstd=q_rstd if fuse_norm_gauge_bwd else None,
            k_rstd=k_rstd if fuse_norm_gauge_bwd else None,
            wy_backward=ctx.implementation.wy_backward,
            intra_backward=ctx.implementation.intra_backward,
        )
        if ctx.use_qk_l2norm_in_kernel:
            if p is not None:
                if not fuse_norm_gauge_bwd:
                    if ctx.implementation.paired_norm:
                        dq, dk = l2norm_gauge_pair_bwd(q, q_rstd, dq, k, k_rstd, dk, p)
                    else:
                        dq = l2norm_gauge_bwd(q, q_rstd, dq, p)
                        dk = l2norm_gauge_bwd(k, k_rstd, dk, p)
            elif not fuse_norm_gauge_bwd:
                dq = l2norm_bwd(q, q_rstd, dq)
                dk = l2norm_bwd(k, k_rstd, dk)
        elif p is not None:
            dq, dk = dq * p.to(dq.dtype), dk * p.to(dk.dtype)
        if ctx.use_beta_sigmoid_in_kernel:
            db = fused_beta_sigmoid_bwd(beta_raw, db, scale=2.0 if ctx.allow_neg_eigval else 1.0)

        return (dq.to(q), dk.to(k), dv.to(v), dg.to(g_input), db.to(beta_raw), None, dA, dbias, None, dh0,
...
def _chunk_kda_impl(
    ...
    **kwargs,
    ...
    Args:
        ...
        sign (Optional[torch.Tensor]):
            Signs of the decay in ``{-1, +1}`` of shape ``[B, T, HV, K]``, for a signed
            ``alpha in [-1, 1]``. ``g`` then carries ``log|alpha|`` and the sign is applied
            as a running product ``P_t = prod_{u<=t} s_u`` on ``q``/``k`` (fused into
            KDA's own l2norm epilogue), with the final state returned un-gauged. Requires
            ``HV == H``, since the gauge is per value head while ``q``/``k`` are shared
            across a GVA group. Default: ``None``.
    ...
    if sign is not None:
        assert sign.shape == (B, T, HV, K), (
            f"sign must have shape [B, T, HV, K]={[B, T, HV, K]}, got {list(sign.shape)}"
        )
        assert HV == H, (
            f"`sign` is per value head but q/k are shared across a GVA group (HV={HV} > H={H}). "
            f"Expand q/k to HV heads, or tie the sign within each group."
        )

    if scale is None:
        scale = K ** -0.5
    implementation = resolve_implementation(
        kwargs.pop('_implementation', None), q, v, use_qk_l2norm_in_kernel, cp_context,
    )
...
def chunk_kda(*args, **kwargs):
    return _chunk_kda_impl(*args, **kwargs)


chunk_kda.__doc__ = _chunk_kda_impl.__doc__
chunk_kda.__signature__ = signature(_chunk_kda_impl)
