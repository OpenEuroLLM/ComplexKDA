...
from fla.ops.kda.gauge import gauge_last, gauge_state, kda_sign_cumprod
    ...
    **kwargs,
    ...
    Args:
        ...
        sign (Optional[torch.Tensor]):
            ...
            `alpha in [-1, 1]`; see `chunk_kda`. Requires `HV == H`. Default: `None`.
    ...
    # Sign gauge. Decode-length, so a plain multiply; the kernel's l2norm commutes with it.
    p = None
    if sign is not None:
        if sign.shape[2] != q.shape[2]:
            raise ValueError(
                f"`sign` is per value head but q/k are shared across a GVA group "
                f"(HV={sign.shape[2]} > H={q.shape[2]}). Expand q/k to HV heads, or tie the sign."
            )
    ...
    if p is not None and final_state is not None:
        final_state = gauge_state(final_state, gauge_last(p, cu_seqlens), state_v_first)
