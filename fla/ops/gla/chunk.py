...
from fla.utils import IS_NVIDIA_HOPPER, autotune_cache_kwargs, check_shared_mem, input_guard
...
def _prune_gla_fwd_o_configs(configs, nargs, **kwargs):
    args = {**(nargs or {}), **kwargs}
    # eight-warp BT=32 launches can fail with misaligned addresses on Hopper.
    if IS_NVIDIA_HOPPER and args['BT'] == 32:
        return [config for config in configs if config.num_warps < 8]
    return configs
...
@fla_cache_autotune(
    ...
    prune_configs_by={'early_config_prune': _prune_gla_fwd_o_configs},
    **autotune_cache_kwargs,
)
@triton.jit(do_not_specialize=['T'])
