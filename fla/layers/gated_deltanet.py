...
def _triton_available() -> bool:
    """True only if a Triton kernel can actually launch, which is stricter
    than `import triton` succeeding -- fla imports fine on CPU and fails at
    call. Mirrors fla.layers.complex_kda_layer.HAS_TRITON."""
    try:
...
class GatedDeltaNet(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        # ShortConvolution and FusedRMSNormGated are triton-only; swap in the
        # Triton where the tensors allow it, pure torch elsewhere -- the same
        # pattern as ComplexKimiDeltaAttention, which now decides per CALL
        # rather than per MACHINE. Choosing on HAS_TRITON alone (it is
        # `torch.cuda.is_available()`) left this layer broken in exactly the
        # case the fallback exists for: a box that HAS a GPU but is handed CPU
        # tensors installed the Triton modules and died at kernel launch.
        from fla.layers.complex_kda_layer import FusedRMSNormGated, ShortConvolution
    ...
    def _use_fused_qkv_conv(
        ...
        #
        # CUDA ONLY, and checked FIRST. `causal_conv1d` is a Triton kernel
        # called directly here, so it bypasses ShortConvolution's own CPU
        # fallback entirely: on CPU tensors this path dies at kernel launch,
        # whatever the convolutions are. Declining it sends CPU tensors through
        # the per-projection branch below, which does fall back. Checking it
        # before anything else also keeps `.backend` untouched on CPU -- the
        # pure-torch ShortConvolutionCPU has no such attribute.
        if not on_cuda:
            return False
    ...
    def forward(
        ...
        **kwargs: Unpack[dict],
        ...
        if self._use_fused_qkv_conv(last_state, use_cache, cu_seqlens, hidden_states.is_cuda):
            qkv = torch.cat(
                [
        ...
        # ON THE TENSORS, NOT THE MACHINE. `HAS_TRITON` is
        # `torch.cuda.is_available()`, so a GPU box handed CPU tensors took the
        # kernel branch and died at launch. `q` is what the op is about to be
        # given, so its device is the question actually being asked.
        if not (HAS_TRITON and q.is_cuda):
            # CPU/no-triton fallback: fla.ops.gated_delta_rule.naive, driven
            # through a pure-PyTorch recurrent loop instead of the fused
            # kernels above. Replicates exactly what use_qk_l2norm_in_kernel /
            # use_gate_in_kernel / use_beta_sigmoid_in_kernel do inside the
            # triton kernels (see fla.ops.gated_delta_rule.{chunk,gate}), just
            # applied in plain torch before the recurrence instead of fused
            # into it. Mirrors fla.layers.complex_kda_layer's naive_kda_call.
            q_n = F.normalize(q.float(), dim=-1)
            k_n = F.normalize(k.float(), dim=-1)
            g_naive = naive_gdn_gate(self.a_proj(hidden_states), self.A_log, self.dt_bias)
            beta_naive = torch.sigmoid(beta.float()) * (2.0 if self.allow_neg_eigval else 1.0)
            o, recurrent_state = naive_recurrent_gated_delta_rule(
                q=q_n, k=k_n, v=v.float(), beta=beta_naive, g=g_naive,
