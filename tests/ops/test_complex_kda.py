...
# ---------------------------------------------------------------------------
# Signed decay gate: alpha in [-1, 1] instead of (0, 1].
#
# This file is a 1:1 mirror of test_kda.py. Every test below is the upstream
# KDA test with ONE extra parametrize axis, `sign_mode`, cross-producted over
# the existing rows by a stacked @pytest.mark.parametrize:
#
#   "unsigned"     sign=None -- reproduces the upstream test exactly
#   "positive"     sign=+1 everywhere -- must equal "unsigned" numerically
#   "alternating"  sign flips every token -- worst case for the parity scan
#   "random"       ~half the (token, channel) pairs negative
#
# The reference stays fla.ops.kda.naive, which knows nothing about signs. The
# sign reaches it as the running gauge P_t = prod_{u<=t} s_u pushed onto q/k,
# with the final state un-gauged at the boundary -- precisely the identity the
# kernel's fused gauge (fla/ops/kda/gauge.py) implements internally. So each
# assert_close below is a real test of that fusion, not a restatement of it.
# The parity here is computed independently of the implementation, so the test
# does not validate the kernel against its own helper.
#
# Two combinations are skipped rather than tested, because the op rejects them:
#   * HV > H (GVA): `sign` is per value head while q/k are shared across the
#     group -- chunk.py asserts HV == H.
#   * use_gate_in_kernel=True: the kernel derives g from A_log/dt_bias with a
#     monotone activation, which cannot carry a sign.
# ---------------------------------------------------------------------------

SIGN_MODES = ("unsigned", "positive", "alternating", "random")


def make_sign(sign_mode, B, T, HV, D, *, H=None, use_gate_in_kernel=False, dev=None, seed=42):
    """A [B, T, HV, D] int8 sign in {-1, +1}, or None for the unsigned arm.

    Skips the combinations the op rejects, so a signed row that cannot be
    expressed is reported as skipped rather than silently passing as unsigned.
    """
    if sign_mode == "unsigned":
        return None
    if H is not None and H != HV:
        pytest.skip(reason=f"`sign` is per value head but q/k are shared (HV={HV} > H={H})")
    if use_gate_in_kernel:
        pytest.skip(reason="use_gate_in_kernel=True derives g in the kernel and cannot carry a sign")
    dev = device if dev is None else dev
    T = int(T)
    if sign_mode == "positive":
        return torch.ones(B, T, HV, D, dtype=torch.int8, device=dev)
    if sign_mode == "alternating":
        t = torch.arange(T, device=dev).view(1, T, 1, 1)
        return torch.where(t % 2 == 0, 1, -1).expand(B, T, HV, D).to(torch.int8).contiguous()
    if sign_mode == "random":
        gen = torch.Generator(device="cpu").manual_seed(seed)
        r = torch.rand(B, T, HV, D, generator=gen)
        return torch.where(r < 0.5, -1, 1).to(torch.int8).to(dev)
    raise ValueError(f"unknown sign_mode {sign_mode!r}")


def running_gauge(sign, cu_seqlens=None):
    """P_t = prod_{u<=t} s_u along time, restarting at each sequence start.

    Computed here rather than imported, so the reference does not depend on the
    implementation under test. A cumulative product of +-1 is exact in float.
    """
    if sign is None:
        return None
    P = sign.float().cumprod(dim=1)
    if cu_seqlens is not None:
        for i in range(len(cu_seqlens) - 1):
            s, e = int(cu_seqlens[i]), int(cu_seqlens[i + 1])
            P[:, s:e] = sign[:, s:e].float().cumprod(dim=1)
    return P


def _ungauge(ht, P_last, state_v_first=False):
...
def naive_recurrent_kda_signed(*, sign=None, cu_seqlens=None, **kw):
    """naive_recurrent_kda with the gauge applied around it."""
    P = running_gauge(sign, cu_seqlens)
    if P is None:
        return naive_recurrent_kda(**kw)
    kw = dict(kw)
    kw["q"], kw["k"] = kw["q"] * P, kw["k"] * P
    o, ht = naive_recurrent_kda(**kw)
    return o, _ungauge(ht, P[:, -1])


def naive_chunk_kda_signed(*, sign=None, cu_seqlens=None, **kw):
    """naive_chunk_kda with the gauge applied around it."""
    P = running_gauge(sign, cu_seqlens)
    if P is None:
        return naive_chunk_kda(**kw)
    kw = dict(kw)
    kw["q"], kw["k"] = kw["q"] * P, kw["k"] * P
    o, ht = naive_chunk_kda(**kw)
    return o, _ungauge(ht, P[:, -1])


@pytest.mark.parametrize("sign_mode", SIGN_MODES)
...
def test_naive_chunk(
    ...
    sign = make_sign(sign_mode, B, T, HV, D, H=H)

    ref, ref_ht = naive_recurrent_kda_signed(
        sign=sign,
    ...
    tri, tri_ht = naive_chunk_kda_signed(
        sign=sign,
        q=F.normalize(q.clone(), p=2, dim=-1),
        k=F.normalize(k.clone(), p=2, dim=-1),
        v=v.clone(),
...
def test_fused_recurrent_use_beta_sigmoid_in_kernel(
    ...
    sign = make_sign(sign_mode, B, T, H, D)
...
def test_fused_recurrent_gate_in_kernel(
    ...
    sign = make_sign(sign_mode, B, T, HV, D, H=H, use_gate_in_kernel=True)
...
def test_fused_recurrent_vllm_decode(
    ...
    if sign_mode != "unsigned":
        pytest.skip(reason="fused_recurrent_kda_fwd takes no `sign`; only the public op does")
...
def test_chunk(
    ...
    sign = make_sign(sign_mode, B, T, HV, D, H=H, use_gate_in_kernel=use_gate_in_kernel)
...
def test_chunk_varlen(
    ...
    sign = make_sign(sign_mode, 1, T, H, D, use_gate_in_kernel=use_gate_in_kernel)
    ...
    ref_ht = []
    for i in range(N):
        ref_i, ref_ht_i = naive_recurrent_kda_signed(
            sign=(None if sign is None else sign[:, cu_seqlens[i]: cu_seqlens[i + 1]]),
...
def test_chunk_return_intermediate_states(dtype, sign_mode: str):
    ...
    with torch.inference_mode():
        ...
        sign_varlen = make_sign(sign_mode, 1, total_tokens, H, D)
